...
public class Relay {
	...
	public static void sendToCore(ZeroMQBaseConnector coreConnector, OFMessage msg, long datapathId, int moduleId) {
		...
		if (lastXID >= 0){
			...
			msg.writeTo(dcb);
			byte[] payload = new byte[dcb.readableBytes()];  //you xiao fu zai
	...
	public static void sendToController(NetIDEProtocolVersion netIpVersion, ZeroMQBaseConnector coreConnector,
		...
		// SEND PACKETS DIRECTLY TO THE PIPELINE 直接发送数据包到管道
		FloodlightContext context = new FloodlightContext();
		
		// TEMP FIX for apps that retrieve payload from FloodlightContext //临时修复应用程序检索的有效载荷从FloodlightContext
		// object, eg, Learning Switch
		if (message.getType().equals(OFType.PACKET_IN) && fenceSupport) {
			logger.info("fencesupport");
			lastXID = netIpID;
			...
			if(moduleName != null && !moduleName.isEmpty() && netIpID != 0) {  //module分配了内存而且有数据
				...
				if (listeners != null) {
					for (IOFMessageListener listener : listeners) {
						if (moduleName.equals(listener.getName())) {
							listenerFound = true;
		                    listener.receive(sw, message, context); //sw receives OFmessage
		                    FenceMessage fence = new FenceMessage();  //module send FenceMessage
